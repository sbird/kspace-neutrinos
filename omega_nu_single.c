...
void rho_nu_init(_rho_nu_single * rho_nu_tab, double a0, double mnu, double HubbleParam)
{
     int i;
     double abserr;
     ...
     for(i=0; i< NRHOTAB; i++){
        ...
        F.params = &param;
        gsl_integration_qag (&F, 0, 500*BOLEVK*TNU,0 , 1e-9,GSL_VAL,6,w,&(rhonu[i]), &abserr);
        rho_nu_tab->rhonu[i]=rho_nu_tab->rhonu[i]/pow(exp(rho_nu_tab->loga[sp][i]),4)*get_rho_nu_conversion();
     }
